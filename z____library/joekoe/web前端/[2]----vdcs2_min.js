...
var VDCS={},vdcs={vers:{bulid:'0.0.1',d:'20160123'}},DCS=dcs={};
var global=window;
...
isb=function(s){return typeof(s)==='boolean'};
iss=function(s){return typeof(s)==='string'};
isd=function(o){return objString.call(o)=='[object Date]'};
isa=function(o){return objString.call(o)=='[object Array]'};
iso=function(o){return objString.call(o)=='[object Object]'};
isoo=function(o){return typeof(s)==='object'};
isf=function(o){return o instanceof Function||typeof(o)=='function'};
tn=function(o){return typeof(o).toString()};


isint=isInt=function(s){return parseInt(s)==s};				isnum=isNum=function(s){return parseFloat(s)==s};
isInte=function(s){return /^[0-9]*[1-9][0-9]*$/.exec(s)?true:false};
...
tob=function(s){return (s==true||s==1||s=='True'||s=='true'||s=='1')};
toi=function(s,b){return isInt(s)?parseInt(s,b||10):0};		ton=function(s){return isNum(s,1)?parseFloat(s):0};

t=function(s){return iss(s)?s.trim():s};					len=function(s){return s.replace(/[^\x00-\xff]/g,'aa').length};
r=function(s,s1,s2){if(!s||ise(s1))return s;var n=0;while(s.indexOf(s1)>-1&&n<100){s=s.replace(s1,s2);n++};return s};
rv=function(s,s1,s2){return r(s,'{'+s1+'}',s2)};			rd=function(s,s1,s2){return r(s,'{$'+s1+'}',s2)};
...
substr=function(s,n,n2){return s?s.substr(n-1,n2-1):s};		split=function(s,p){return s?s.split(p):[]};
strl=left=function(s,n){return s?s.substr(0,n):s};			strr=right=function(s,n){return s?s.substr(s.length-n):s};
...
ina=function(ary,v){return $.inArray(v,ary)+1};				ino=function(obj,k){return obj.hasOwnProperty(k)};

vv=function(s,v){return isn(s)?v:s};
upper1=function(s){return s?(s.substring(0,1).toUpperCase()+s.substring(1,s.length)):s};

eacha=function(a,fn){for(var n in a){fn(a[n],n+1)}};		eacho=function(o,fn){var n=0;for(var k in o){if(o.hasOwnProperty(k)){fn(k,o[k],n+1);n++}}};
eachx=function(o,fn){return $.each(o,fn)};


cloner=function(o){return vdcs.cloner(o)};
extendx=ox=function(o,a,b,c,d,e){return Object.assign({},o,a,b,c,d,e)};
extendo=function(o,a,b,c,d,e){return Object.assign(o,a,b,c,d,e)};

iscfg=isopt=function(o){return (o instanceof Object || o instanceof Array)};
oxx=function(o,a,b,c,d,e){return Object.assign(true,{},o,a,b,c,d,e)};


$jo=function(o){o=$(o);if(!o.length)o=null;return o};


o2s=function(o,p1,p2,f){return utilString.toString(o,p1,p2,f)};		s2o=function(s,p1,p2,f){return utilString.toObject(s,p1,p2,f)};
json2o=json_decode=function(s){return utilJSON.toObject(s)};		o2json=json_encode=function(o){return utilJSON.toString(o)};
url_decode=function(o){return $url.de(o)};							url_encode=function(o){return $url.en(o)};


islocal=isLocal=function(){return vdcs.isLocal()};			isMobile=function(){return vdcs.isMobile()};


query=function(k){return dcs.query(k)};

refresh=function(){return dcs.refresh()};
go=function(url){return dcs.go(url)};						goback=function(){return dcs.goback()};
gox=function(url){return dcs.gox(url)};						goi=function(url){return dcs.goi(url)};
...
extendo(vdcs,{
	...
	serveURL:function(v){return $serve.url(v)},


	isDebug:function(v){return this.isdebug(v)},isdebug:function(v){var r=this.DEBUGV;if(typeof v=='undefined' && r) return true;return v==r},
	isLocal:function(){return this.is_local},islocal:function(){return this.is_local},
	isMobile:function(){return this.device=='mobile'},
	isWX:function(){if(typeof $wx != 'undefined'){return $wx.is()}else{return (this.agent.match(/MicroMessenger/i)=='micromessenger')?true:false}},
	...
	vert:function(t,real){var re=(this.is_local||real)?Date.d():dcs.vers.d;return t?('_t='+re):re},
	...
	nav:navigator,agent:navigator.userAgent.toLowerCase(),ver:navigator.appVersion,
	prober:function(){
	...
	urler:function(){
		if(this.init_url) return;this.init_url=true;
		var au={};var wl=window.location;
		au['title']=document.title;au['referrer']=au['referer']=wl.referrer||document.referrer;au['uri']=au['href']=wl.toString();/*d.URL*/
		au['protocol']=wl.protocol.substr(0,wl.protocol.length-1);au['port']=wl.port;
		au['domain']=wl.hostname;au['path']=wl.pathname;au['paths']=au['path']+wl.search;
		au['query_string']=wl.search?wl.search.substr(1):'';au['hash']=wl.hash?wl.hash.substr(1):'';
		au['url']=au['urlbase']=wl.origin+'/';au['urls']=au['urlbase']+au['path'].substr(1);
		au['channel']='';au['dirn']=0;for(var a=0;a<5;a++){au['dir'+(a+1)]=''};
		var ary=au['path'].split('/');
		if(ary.length>2){
			au['channel']=ary[1];
			au['dirn']=ary.length-2;
			for(var a=2;a<(ary.length-1);a++){au['dir'+(a-1)]=ary[a]}
			au['script']=ary[ary.length-1];
		}
		else{au['script']=ary[1]}
		var il=au['script'].lastIndexOf('.');
		au['page']=il>0?au['script'].substr(0,il):au['script'];au['ext']=il>0?au['script'].substr(il+1):'';
		this._url=au;
	},urls:function(){this.urler();return this._url},
	url:function(k){this.urler();return this._url[k||'uri']},

	// query
	urlHash:function(opt){
		var hasha=this.url('hash').split('@');
		var hash=hasha[hasha.length-1],token='';
		if(hasha.length==2) token=hasha[0];
		return {hash:hash,token:token}
	},

	query:function(k){return $.query.get(k)},

	// cookie
	cookie:function(k,v,epr){return isn(v)?this.cookieSet(k,v,epr):cookieGet(k)},
	cookieSet:function(k,v,epr){
		var _day=0;
		switch(epr){
			case 'day':		_day=1;break;
			case 'week':	_day=7;break;
			case 'month':	_day=30;break;
			case 'year':	_day=365;break;
			case 'yes':		_day=3650;break;
		}
		if(_day>0){
			var d=new Date();
	...
	// other
	append:function(type,value,opt){return $.append(type,value,opt)},
	open:function(u,n,wid,hei,scroll,cont){var l=(screen.width)?(screen.width-wid)/2:0,t=(screen.height)?(screen.Height-hei)/2:0;var o=window.open(u,n,'width='+wid+',height='+hei+',left='+l+',top='+t+',toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars='+scroll+',resizable=no');if(cont){o.document.write(cont)}return o},

	timeout:function(n,f){return setTimeout(f,n*1000)},timeoutClear:function(t){return clearTimeout(t)},
	interval:function(n,f){return setInterval(f,n*1000)},intervalClear:function(t){return clearInterval(t)}
});
$(function(){vdcs.initer()});


global.newEvent=function(){return new VDCS.utilEvent()};
global.newMap=function(){return new VDCS.utilMap()};
global.newTable=function(){return new VDCS.utilTable()};global.isTable=function(o){return isot(o,'VDCS.utilTable')};
...
global.util=$util={
	...
	toRegex:function(ptn){return iss(ptn)?new RegExp(ptn,'gi'):ptn},
	replaceRegex:function(ptn,re,values,fn){fn=fn||function(s,k){return values[k]};return re.replace(this.toRegex(ptn),fn)},
	replacePre:function(re,values){return this.replaceRegex(this.PATTERN_PRE,re,values)},
	replaceVar:function(re,values){return this.replaceRegex(this.PATTERN_PRE,re,values)},
	replaceFilter:function(re,values,ptn,fn){if(!re) return'';return re.replace(ptn||this.PATTERN_LEBEL_ITEMS,function(s,field,s2,fmt,s4,s5){return util.filterValue(values[field],fmt,s5,fn)})},
	...
	toMap:function(o){return (iso(o)&&o.TYPENAME=='utilMap')?o:utilJSON.toMap(o)}
};

VDCS.Error=function(){
	var _n=0;_dat=[];
	this.is=function(){return (_n>0)?false:true};
	this.add=function(s){_n++;_dat[_n]=s};
	this.toString=function(){var rea=[];for(var i=1;i<=_n;i++){rea.push(_dat[i])};return rea.join("\n")};
	this.pop=function(){if(_n>0)alert(this.toString())};
};


VDCS.utilVar=function(){
	this.__vars_={};
	this.set=function(k,v){this.__vars_[k]=v;return this};
	this.get=function(k){return this.__vars_[k]}
};

VDCS.utilEvent=function(){
	this.__events_={};
	this.is=function(a){return this.isEvent(a)};
	this.isEvent=function(action){return !!this.__events_[action]};
	this.getEvent=function(action){return this.__events_[action]};
	...
	this.autorun=function(){
		if(this.opt && this.opt.autor){
			var that=this;
			if(this.run) vdcs.timeout(0.1,function(){that.run()});
...
global.newDate=function(d){return new VDCS.utilDate(d)};
...
VDCS.utilDate=function(d){
	...
	this.timer=function(){return Math.round(this.d.getTime()/1000)};
	this.num=function(){var re=Date.parse(this.d)/1000;if(re<1) re=0; return re};
	this.now=function(){return this.format('time')};

	this.format=function(type,real){
		var re=this.fmt_def;
		switch(type||'time'){
			case 'time':		re='yyyy-mm-dd hh:ii:ss';break;
			case 'timec':		re='yyyy-mm-dd hh:ii';break;
			case 'timei':		re='yy-mm-dd hh:ii';break;
			case 'his':			re='hh:ii:ss';break;
			case 'ymd':
			case 'day':
			case 'date':		re='yyyy-mm-dd';break;
			case 'datei':		re='mm-dd hh:ii';break;
			case 'dates':		re='yyyy-mm-dd hh:ii';break;
	...
	this.toString=function(){
		...
		var _year=this.d.getFullYear().toString(),_month=this.d.getMonth()+1;
		re=r(re,'yyyy',_year);				re=r(re,'yy',_year.substr(2));
		re=r(re,'mm',this.v2(_month));		re=r(re,'m',_month);
		re=r(re,'dd',this.v2(this.d.getDate()));		re=r(re,'d',this.d.getDate());
		re=r(re,'hh',this.v2(this.d.getHours()));		re=r(re,'h',this.d.getHours());
		re=r(re,'ii',this.v2(this.d.getMinutes()));		re=r(re,'i',this.d.getMinutes());
		re=r(re,'ss',this.v2(this.d.getSeconds()));		re=r(re,'s',this.d.getSeconds());
		return re
	};

	this.distances=['@','$1秒前','$1分钟前','$1小时前','$1天前'];
	this.distance=function(){
		var n=this.num(),re=[0,'',-1];
		if(n>86400){re[0]=4;re[2]=parseInt(n/86400)}
		else if(n>3600){re[0]=3;re[2]=parseInt(n/3600)}
		else if(n>60){re[0]=2;re[2]=parseInt(n/60)}
		else if(n>0){re[0]=1;re[2]=n}
		if(re[0]>0){
			if(re[2]<1)re[2]=1
	...
	this.v2=function(s){if(s.toString().length<2)s='0'+s;return s};
};


global.utilString=VDCS.utilString={

	toString:function(o,p1,p2,f){p1=p1||';';p2=p2||'=';f=f||function(v){return v};var a=[];eacho(o,function(k,v){a.push(k+p2+f(v))});return a.join(p1)},
	toObject:function(s,p1,p2,f){s=s||'';p1=p1||';';p2=p2||'=';f=f||function(v){return v};var re={},ar,ars=s.split(p1),alen=ars.length;for(var i=0;i<alen;i++){ar=ars[i].split(p2);if(ar.length==2&&t(ar[0])!='') re[t(ar[0])]=f(t(ar[1]))}return re},
...
global.utilJSON=VDCS.utilJSON={
	...
	toString:function(o){return this.encode(o)},
	toObject:function(s,b){var o=b?{}:null;return this.decode(s)||o},

	encode:function(o,r){try{return iso(o)?JSON.stringify(o):o}catch(e){return r?'{}':null}},
	decode:function(s,r){try{return iss(s)?JSON.parse(s):s}catch(e){return r?{}:null}},
	...
	result:function(jo,json){
		jo.css({'textAlign':'left','whiteSpace':'pre','fontFamily':'monospace'});
...
vBasic.fn.extend({
	resultJSON:function(json){return utilJSON.result(this,json)}
});


global.newTable=function(v){return new VDCS.utilTable(v);};
global.isTable=function(obj){return obj&&obj.TYPENAME=='utilTable'};
...
VDCS.utilTable=function(ary){
	...
	this.each=function(fn){return this.eachObject(fn)};
	this.eachJSON=function(fn){return this.eachObject(fn)};
	this.eachRow=function(fn){return this.eachObject(fn)};
	...
	this.rows=function(){return this._data.length};
	...
	*/
	...
	this.setItem=this.set=function(index,obj){
		this._data[index]=obj;return this
	};
	...
	########################################
	*/
	this.getFields=function(){return this.getFielda().join(',')};
	this.getFielda=function(){
		if(this.isEmpty()) return [];
		return Object.keys(this._data[0]);
	};

	this.getValues=function(field,smb){return this.getValuea(field,smb).join(',')};
	...
	this.appendTable=function(table2,opt){
		...
		var items={};
		var filter_row=opt.filter_row||function(row){return row};
		var filter_item=opt.px?function(json){return utilJSON.objextPX(json,opt.px)}:function(json){return json};
	...
	*/
	...
	this.toResults=function(){return this.toArray()};

	this.setJSON=function(json){return this.setArray(json)};
	this.toJSON=function(){return this.toArray()};

	this.toString=function(){return o2json(this.toJSON())};

	this.render=function(opt){return ui.render.tableReplace(this,opt)};
...
VDCS.utilMap=function(obj){
	...
	this.setObject=function(obj){this._obj=cloner(obj)||{};return this};
	this.setObject(obj);

	this.isEmpty=function(){return this._dat.length>0?false:true};

	this.is=function(k){return !isun(this._dat[k])};
	this.add=function(k,o){this._dat[k]=o;return this};
	this.set=function(k,o){this._dat[k]=o;return this};
	this.get=function(k,o){return this._dat[k]};

	this.heads=function(){return this.get('head')};
	this.vars=function(){return this.get('var')};
	this.lists=function(k){return this.getJSON(k)};
...
VDCS.baseServe=function(){
	// options
	this.filterOpt=function(o){return o};
	this.filterSelector=function(o){return o};

	// serve
	this.isServePost=function(){return this.opt.serve_method=='post'};
	this.serveVars=function(k,v){this.opt.serveVar.vars[k]=v;return this};
	this.serveVarFilter=function(vars){return vars};
	this.serveURL=function(){return this.opt.serveURL||vdcs.serveURL(this.serveVarFilter(this.opt.serveVar))};
};

VDCS.serve=function(vars){
	this._var=vars||{entry:'entry',api:'status',x:'json',action:''};
	this._uri='/{entry}/';this._urlx='{api}.{x}?action={action}';
	this._mode='channel';
	this.uri=function(v){return this._uri=v;return this};
	this.serve=function(v){return this._var.x=v;return this};
	this.isJSONP=function(){return this._var.x=='jsonp'};
	...
	this.setChannel=this.setEntry=function(v){return this.setVar('entry',v)};
	this.setMode=function(mode){this._mode=mode;return this};
	this.modeURL=function(mode,v){
		var url=this._uri;
		switch(mode){
			case 'api':			url='/api/{entry}/';break;
			case 'define':		url=this._uri;break;
		}
		return url
	};
	this.filterURL=function(url){for(var i=0;i<3;i++){url=r(url,'/.','.')}return url};
	...
	this.xparami=function(ps){this._var.parami=ox(this._var.parami,ps);return this};
	this.xparams=function(ps){this._var.params=ox(this._var.params,ps);return this};
	this.xvars=function(ps){this._var.vars=ox(this._var.vars,ps);return this};
};
...
// request
global.$request=function(){return util.request.apply(util,arguments)};

extendo(util,{
	request:function(serveVar,opt,fn){
		var that=this;
		if(!iso(serveVar)) serveVar={api:serveVar};
		if(!fn){fn=opt;opt={}}
		opt=ox({format:'map',succeed:'succeed'},opt);
		if(opt.action) serveVar.action=opt.action;
		var url=opt.url||$serve.getURL(serveVar);
		var ajax=$ajax({progress:opt.progress,url:url,send:opt.post||opt.send,format:opt.format})
		.on('ready',function(result){that.request_parser(result,opt,fn,this)})
		;return ajax
	},
...
vdcs.storager={
	driver:function(driver){
		var obj=null;
		switch(driver){
			case 'session':				obj=this.session();break;
			case 'cache':				obj=this.cache();break;
			default:					obj=this.local();break;
		}
		return obj
	},
...
utilStorage.prototype={
	initer:function(container){
		switch(container||this.opt.container){
			case 'session':			this._cache=window.sessionStorage;break;
			case 'cache':			this._cache=window.cacheStorage;break;
			case 'local':
			default:				this._cache=window.localStorage;break;
		}
	},


	exist:function(key){return !!this._cache[key]},
	count:function(){return this.clear.length},
	remove:function(key){this._cache.removeItem(key);return this},
	clear:function(){this._cache.clear();return this},
	getValue:function(key){return this._cache.getItem(key)},
	...
	getObject:function(key){return utilJSON.toObject(this.getValue(key))},


	info:function(key){return this.getValue(key)},
	is:function(key){
	...
	hashGets:function(key){return this.get(key)},
	...
	getKeys:function(){return this.getKeyAry().join(',')},
	getKeyAry:function(){
		var ary=[];
	...
	getTable:function(){return this.toTable()},
	toTable:function(){
...
global.utilSession=function(node){
	this.opt={node:node,drive:'local'};
};
utilSession.prototype={
	setTemp:function(){this.opt.drive='session';return this},
	driver:function(){
		this.odriver=this.odriver||vdcs.storager.driver(this.opt.drive);
		return this.odriver
	},
	get:function(key){var re=this.driver().hashGet(this.opt.node,key);if(isun(re)) re='';return re},
	set:function(key,value){this.driver().hashSet(this.opt.node,key,value);return this},
	remove:function(key){this.driver().hashDel(this.opt.node,key);return this},
};


VDCS.SocketIOClient=function(){
	VDCS.utilVar.call(this);VDCS.utilEvent.call(this);
	this.debug=function(bool){this.set('debug',bool);return this};
	this.server=function(url){this.set('url',url);return this};
	this.token=function(token){this.set('token',token);return this};
	this.connect=function(url){
		...
		sock
		.on('connect',function(){that.emit_sub('base.connect',arguments);that.emit_sub('filter.connect',arguments)})
		.on('disconnect',function(){that.emit_sub('base.disconnect',arguments);that.emit_sub('filter.disconnect',arguments)})
		.on('auth',function(){that.emit_sub('base.auth',arguments);that.emit_sub('filter.auth',arguments)})
		.on('data',function(){that.emit_sub('base.data',arguments);that.emit_sub('filter.data',arguments)})
		.on('status',function(){that.emit_sub('base.status',arguments);that.emit_sub('filter.status',arguments)})
		.on('error',function(){that.emit_sub('base.error',arguments);that.emit_sub('filter.error',arguments)})
		;
		this.sock=sock;
		return this
	};
	...
	this.driver=function(){return this.sock};
...
String.prototype.r=function(a,b){return r(this,a,b)};
String.prototype.toJSON=function(){return utilJSON.toObject(this.toString())};
String.prototype.toObject=function(p1,p2){return utilString.toObject(this,p1,p2)};
String.prototype.url=function(b){return dcs.url(this,b)};


DCS=dcs={
	today:function(){},
	now:function(){},
	timem:function(d){return (d||new Date()).getTime()},

	query:function(k){return vdcs.query(k)},queryi:function(k){return toi(vdcs.query(k))},

	url:function(s,b){return utilURL.base(s,b||this.BROWSE_HOST||vdcs.url('urlbase'))},

	refresh:function(){location.reload()},
	go:function(s){location.href=s},
	goback:function(){return w.history.back()},
	gox:function(s){if(vdcs.isdebug()){dbg.t('url',s)}else{go(s)}},
	goi:function(s){if(vdcs.isLocal()||vdcs.isdebug()){dbg.t('url',s)}else{go(s)}}
};


// dbg debug
global.dbg={
o:function(o,fn){isa(o)?this.ary(o,fn):this.obj(o,fn)},
obj:function(obj,fn,n){
	fn=fn||function(a){alert(a.join(EOL))},n=n||30;
	var rea=[],t=0;
	eacho(obj,function(k,v){
		rea.push(k+' = '+(isf(v)?'[Function]':v));t++;
		if(rea.length>n){fn(rea);rea=[]}
	});
...
url:function(s){this.t('url',s)},hold:function(s){this.t('HOLD',s)},
t:function(t,s){
	var v1='',v2='';
	t=t.toString();
	if(isn(s)){v2=t}
	...
	var _in=function(v){return that.jin.prepend('<p>'+v1+v+'</p>')};
	if(iso(v2)) eacho(v2,function(k,v,n){_in('['+n+'] '+k+' = '+v)});
	else if(isa(v2)) eacha(v2,function(v,n){_in('['+n+'] '+v)});
	else _in(v2);
},
box:function(opt){
	opt=ox({id:'test-box',place:'left',zIndex:999},opt);
	var id=opt.id,o=$('#'+id);
	...
	o.initi(function(){
		...
		var space=dbg.space();
		var csso={
			maxHeight:'80%',padding:'6px 10px',
			color:'rgba(0,0,0,.7)','text-align':'left','line-height':1.5,
			backgroundColor:'rgba(255,255,255,.7)','border-radius':5,
			position:'fixed',bottom:$('#footbar_holder').height()||space,zIndex:opt.zIndex
		};
...
space:function(){return vdcs.isMobile()?10:20},
logi:function(s){if(isdebug('log')) this.log(s)},loga:function(s){if(isdebug('log')) this.loga(s)},
log:function(){console.log.call(console,vdcs.args_unpush(arguments,'DEBUG'))}
};
global.isdebug=function(v){return vdcs.isdebug(v)};

if(!global.console) global.console={log:function(){},assert:function(){}};


// url
global.$url={
	is:function(s){return this.isValid(s)},isValid:function(s){return (s.indexOf('://')!=-1||s.substring(0,1)=='/')?true:false},
	isSafe:function(s){return regEx.exec(new RegExp("^(http(s?)|ftp)://","i"))},
	querys:function(a){return iss(a)?a:o2s(a,'&','=',function(v){return $url.en(v)})},
	...
	en:function(s){return encodeURIComponent(s)},encode:function(s){return this.en(s)},encodei:function(s){return encodeURI(s)},
	de:function(s){return decodeURIComponent(s)},decode:function(s){return this.de(s)},decodei:function(s){return decodeURI(s)}
};


// code & filter
global.$code=global.$filter={
	bit:function(re,p){return re?'1':'0'},
	bool:function(){return re?'true':'false'},
	...
	len:function(str){return str.replace(/[^\u0000-\u00ff]/g,'aa').length},
	txt:function(re){return re=re.replace(/<[^>]*>/g,'')},
	xml:function(s){return s.replace(['&','"',"'",'<','>','’'],['&amp;','&quot;','&apos;','&lt;','&gt;','&apos;'])},
	text:function(s,p2){return this.toHTML(s,1,p2)},
	html:function(s,p2){return this.toHTML(s,1,p2)},
	message:function(s,p2){return this.toHTML(s,2,p2)},
	explain:function(s,p2){return this.toHTML(s,2,p2)},
	summary:function(s,p2){return this.toHTML(s,2,p2)},
	remark:function(s,p2){return this.toHTML(s,0,p2)},


	md5:function(s){return $.md5?$.md5(s):s},
	md5i:function(s){return this.md5(s).substr(8,16)},


	// number
	toNumber:function(s,n){
		var re='',s=''+s+'',len=s.length,dotp=s.indexOf('.',0);
		if(dotp==-1){
			re=s+'.';
			for(i=0;i<n;i++){re+='0'}
		}
	...
	amount:function(s){return this.toNumber((isNum(s)?s:''),2)},
	price:function(s){return this.amount(s)},
	money:function(s){return this.amount(s)},
	emoney:function(s){return this.toNumber((isNum(s)?s:''),1)},
	commak:function(str){
	...
	// time
	date:function(s){return newDate(s).format('date',true)},
	datei:function(s){return newDate(s).format('datei',true)},
	dates:function(s){return newDate(s).format('dates',true)},
	time:function(s,p1){return newDate(s).format(p1||'time',true)},
	timec:function(s){return newDate(s).format('timec',true)},
	timei:function(s){return newDate(s).format('timei',true)},
	time_dist:function(s){
		var d=newDate(s),dist=d.distance();
	...
	upthumb:function(re,p){return this.upload(re,p||'thumb')},
	upthumbm:function(re,p){return this.upload(re,p||'thumbm')},
...
global.$ajax=function(op_,fn,op2){
	if(fn) op_=ox({url:op_,ready:fn},op2);
	var opt=ox({realtime:true,method:'',url:'',post:'',progress:false},op_);
	opt.ready=opt.ready||opt.succeed;
	var url=opt.url;if(!url) return dbg.log('ajax url is empty.');
...
vBasic.fn.extend({
	xform:function(opt,selector){
		...
		xform.initer(ox({jwrap:jwrap},opt),selector);
		return xform
	}
});
...
global.$form=extendo(newEvent(),{
	...
	ajaxAsync:function(maps,opt,ev){
		if(isf(opt)) opt={succeed:opt};
		opt=ox({autor:true},opt);
		ev=ev||newEvent();
		ev.opt=opt;
		ev.maps=util.toMap(maps);
		ev.run=function(){
			if(!this.is_init || this.is_run)return this;this.is_run=true;
	...
	verify_login:true,
	isLogin:function(){return this.verify_login?ua.isLogin(true):true},
	send:function(){
		if(!this.isLogin())return;
		var that=this;
		if(!this.formCheck()) return;
		if(this.issend) return;this.issend=true;
		var url=this.getURL('send'),_send=this.form_data;
		if(!url) return ui.tips('info','$form.send URL Required!',true);
		$ajax({url:url,send:_send,ready:function(o){that.sendAsync(o)},error:true})
	},
	...
	statusOpt:function(opt){opt=ox({tips:'tips',already:false},opt);return opt},
	...
	statusParser:function(heads,opt){
		heads=iss(heads)?{status:heads}:heads;
		opt=this.statusOpt(opt);
		var status=opt.status||heads.status,msg=this.statusMessage(heads)||opt['message_'+status]||this.statusString(status);
		var tips_status=opt['tips_status_'+status]||'info';
		// dbg.t('tips_status',tips_status);dbg.t('msg',msg);
		if(opt.tips){
			switch(opt.tips){
				case 'mini':	ui.mini.show(msg);break;
				default:		ui.tips(tips_status,msg);break;
			}
		}
	...
	statusString:function(status){
		var re='';
		switch(status){
			case 'init':			re='请求初始化！';break;
			case 'parser':			re='无效的请求解析！';break;
			case 'already':			re='请求已处理！';break;
			case 'processed':		re='请求已处理！';break;
			case 'auth':			re='身份验证失败！';break;
			case 'expired':			re='请求已过期！';break;
			case 'module':			re='无效的模块！';break;
			case 'father':
			case 'root':			re='无效的模块数据！';break;
			case 'params':			re='缺少必要的参数！';break;
			case 'data':			re='缺少必要的数据！';break;
			case 'nodata':			re='无效的数据请求！';break;
			case 'record':
			case 'exist':			re='记录已存在！';break;
			case 'norecord':
			case 'noexist':			re='记录不存在！';break;
			case 'power':
			case 'popedom':
			case 'nopermission':	re='权限不足！';break;
			case 'noown':			re='所有者不匹配！';break;
			case 'own':				re='对象不能为自己';break;
			case 'locked':			re='已锁定记录！';break;
			case 'system':			re='系统记录！';break;
			case 'error':			re='请求异常！';break;
			case 'failed':			re='请求失败！';break;
			case 'succeed':			re='请求成功！';break;
			default:				re='未知状态['+status+']';break;
		}
		return re
	}
});
...
VDCS.forms=function(opt,selector){
	...
	this._initer=function(opt,selector){
		if(this.is_inite)return;this.is_inite=true;

		this.opt=ox(this.opt,opt);this.opt=this.filterOpt(this.opt);
		this.selector=ox(this.selector,selector);this.selector=this.filterSelector(this.selector);
		// dbg.o(this.opt);
		...
		var serveVar={api:this.jform.attrd('serve-api')||'',action:this.jform.attrd('serve-action')||''};
	...
	// message
	this.getMessage=function(key){return this.filterMessage(this.opt['message_'+key] || '['+key+']')};
	...
	this.getValues=function(){return this.jform.formJSON()};
	this.setValues=function(values){this.jform.formValues(values);return this};
	...
	this.urlBack=function(url){if(url) this.opt.backurl=url;return this.opt.backurl||vdcs.url('root')};

	this.formCheck=function(){return $form.formCheck(this)};
	...
	this.submitInit=function(opt){
		...
		var _submit=function(){that.submit();return false};
	...
	this.submitOn=function(){this.submitSet('on')};
	this.reset=function(form){
		if(!isun(form)) from=true;
		if(form) this.jform.each(function(){this.reset()});
		this.submitOn()
	};
	...
	this.parser=function(){
		...
		this.submitSet('ing');
		var that=this;
		var url=this.serveURL(),post=null;
		if(!url){
		...
		this.ajax=$ajax({
			...
			url:url,
			post:post,
			ready:function(o){that.parserAsync(o)}
		})
		.on('error',function(err){that.emit('error',err)});
		return this
	};
	this.parserAsync=function(result){
		...
		var that=this;
		this.emit('results',result);this.emit('result',result);
		this.result=result;
	...
	this.parser_tips=function(status,opt){
		var that=this;
		opt=ox({message:this.getMessage(status)},opt);
	...
	this.isTips=function(status){return this.opt.tips_is && this.opt['tips_'+status]};
	this.tipsMessage=function(msg){this.otips.ihtml(msg)};
	this.otips=null;
	this.tips=function(status,message,fn,timer){
		...
		if(this.isEvent('tips')){
			...
			else this.otips=ui.hint(this.jtips,message,{status:_status,callback:fn,tip:'.itip',tip_class:'itip',speed:this.opt.tips_speed,timer:timer,cover:this.opt.tips_cover});
		}
		return this
	};
};
...
vBasic.fn.extend({
	xlist:function(opt,selector){
		...
		xlist.initer(ox({jwrap:$(this)},opt));
		return xlist
	}
});
...
VDCS.list=function(opt){
	...
	this.opt=ox({
		node_table:'lists',ele_item:null,
		serveVar:{parami:{},params:{}},
		templater:true,
		swiper:false,swiper_power:false,
		progress:true,
		autor:true
	},opt);
	...
	this.page=1;
	this.getURL=function(){return $url.link(this.serveURL(),{page:this.page})};
	...
	this.initer=function(opt){
		...
		if(this.opt.swiper && ui.swiper){
			...
			ui.swiper.up(function(e){that.emit('swipe.up',e)});
		}
	...
	this.render=function(jo){ui.render(jo)};
	this.clear=function(){this.jcont.htmlClear()};
	this.reset=function(){this.clear()};
	this.refresh=function(){this.parser()};
	this.refreshNext=function(){this.clickPage(this.page+1)};
	...
	this.parser=function(){
		...
		if(this.is_parser)return;this.is_parser=true;
		...
		var that=this;
		//this.action='query';
		this.ajax=$ajax({url:this.getURL(),ready:function(o){that.parseAsync(o)}})
			.on('error',function(err){that.emit('error',err)});
		return this
	};
	...
	this.parsing=function(jcont){
		...
		if(this.is_parsing_on) return;this.is_parsing_on=true;
	...
	this.loadon=function(){
		...
		this.jloader=this.jloader||ui.render.loaderFiller(this.jwrap,{jcont:this.jcont,progress:this.opt.progress});
...
global.ui={
	...
	data:function(type,status,msg){
		if(isun(msg)){msg=status;status='';}
		this._dat.push({type:type,status:status,message:msg});
		return this
	},
	dataRender:function(next){
		next=next||function(dat){console.table(dat)};
		next(this._dat);
	},
	...
	resized:function(fn){$.resized(fn)},resizer:function(fn){$.resizer(fn)},
	scrolled:function(fn){$.scrolled(fn)},scroller:function(fn){$.scroller(fn)},

	initer:function(){
		ui.swiper=$.toucher;
	},

	initor:function(node){if(this[node] && this[node].initer) this[node].initer()},
...
extendo(ui,{
	go:function(url,msg){ui.mini.show(msg||'载入中..');go(url)},
	gox:function(url,msg){ui.mini.show(msg||'载入中..');gox(url)},
	goi:function(url,msg){ui.mini.show(msg||'载入中..');goi(url)},
	refresh:function(msg){ui.mini.show(msg||'正在刷新..');refresh()},
	include:function(url,msg){ui.mini.show(msg||'加载中..');util.include(url)},

	// cover
	cover:{_id_:'_ui_cover_',
		...
		show:function(opt){
			...
			if(!this._init){
				this.jo=$('<div></div>').attr('id',this._id_).addClass('ui_cover')
						.css({zIndex:900,position:'fixed',top:0,left:0,backgroundColor:'rgba(0,0,0,.8)',display:'none'})
						.appendTo('body');
				this._init=true
			}
	...
	// ani
	ani:{
		replay:function(jwrap){var that=this;this.reset(jwrap);vdcs.timeout(0.1,function(){that.play(jwrap)})},
		play:function(jwrap){
			jwrap.find('[ui-anim]').each(function(){
				var jo=$(this);
				var _play=function(jo){jo.addClass('animater').addClass('ani-'+jo.attr('ui-anim')).show();};
				vdcs.timeout(jo.attr('anim-timeout')||1,function(){_play(jo)});
			});
	...
	tips:function(){return this.popup.apply(this,arguments)},

	// tip
	tipup:function(a,b,c){return this.tipup.show(a,b,c)},
	tipmini:function(a,b,c){return this.tipmini.show(a,b,c)},
	minipop:function(a,b,c){return this.tipmini.show(a,b,c)},

	// popup
	popups:function(){return this.popup.apply(this,arguments)},
	popup:function(opt,msg,cover,timer,close){
		if(!iso(opt)){
			if(isf(cover)){close=cover;cover=true;}
			opt={status:opt,message:msg};
	...
	hint:function(jo,msg,opt){
		...
		if(this.hintTimer){
			vdcs.timeoutClear(this.hintTimer);this.hintTimer=null;
		}
		if(msg=='hide') return;
	...
	confirm:function(msg,fn,opt){
		if(msg && fn){
			opt=ox({message:msg,callback:fn},opt)
		}
		else{
			opt=msg
		}
	...
	mtip:function(opt){
		...
		if(!opt.onback) opt.onback=function(){$p.goback()};
		jbtn.ibtn('index',opt);jbtn.ibtn('back',opt);
		this.jmtip.opt=opt;
		return this.jmtip
	},
...
ui.itip={
	...
	show:function(){
		var st,msg,opt;
		var args=arguments;
		if(iss(args[1])){msg=args[1];opt=args[2];}
		else{msg=args[0];opt=args[1];}
		opt=ox({timer:2},opt);
		if(!this.jo){
			this.jo=$('<div></div>').attr('id','__itip_'+this._rc_).addClass(this._class_)
					.css({zIndex:this.opt.index,position:'fixed'})
					.appendTo('body');
	...
	hide:function(timer){
		if(!this.jo) return;
		var that=this;
		vdcs.timeout(timer,function(){ui.hide(that.jo,that._rc_)})
	}
};
...
extendo(ui.popup,{
	_id_:'__xtip_popup',_class_:'ui-popup',_class_now:'',speed:500,timer:2,
	init:function(){
	...
	status:function(v,def){var re=v;if(inp('succeed,error,info,load',v)<1) re=def||'info';return re},
	display:function(opt){return this.show(opt,true)},
	show:function(opt,alway){
		this.init();
		if(this._show && !alway)return;this._show=true;
		this.opt=ox({status:'',message:'Hi Joekoe!',cover:true,timer:this.timer,classname:this._class_},opt);
		var that=this;
		var _resize=function(sizes){
			if(!that._show)return;
			that.jo.css({top:(sizes.height-that.jo.height()-100)/2,left:(sizes.width-that.jo.outerWidth())/2});
		};
		if(!this.init_){
			...
			this.jo.on('click',function(){that.click()});
			this.init_=true
		}
		...
		if(this.opt.cover) ui.cover.show({real:true});
		this.message(this.opt.message);
		$.resized(_resize);
		vdcs.timeoutClear(this._timer);
		this.timer_v=this.opt.timer;
		if(this.timer_v) this._timer=vdcs.timeout(this.timer_v,function(){this.timer_v,that.hide()});
		return this.jo
	},
	message:function(msg){
		ui.data('popup',msg);
	...
	click:function(){if(this.timer_v) this.hide();return this},
	hide:function(){
...
ui.masker=function(opt){
	...
	ui.popup(ox(opt,{timer:0,cover:true}));
};
	...
	show:function(opt){
		...
		if(!this._init){
			this.jo=$('<div></div>').attr('id',this._id_).append('<div class="iload x64 cc"></div>')
					.css({zIndex:950,position:'fixed',top:0,left:0,width:'100%',height:'100%',display:'none'})
					.appendTo('body');
...
extendo(ui.render,{
	logi:function(k,v){return dbg.log('ui.render',k,v)},

	replaceItem:function(re,items,pattern){return util.replaceFilter(re,items,pattern)},
	...
	jsonReplace:function(json,thos){
		...
		thos.opt=ox({mode:'input',selector:$form.selector,sel:'name'},thos.opt);		//:input
		var opt=thos.opt;
		thos.jwrap=thos.jwrap||$jo(opt.jwrap||opt.wrap);
		if(!thos.jwrap) return this.logi('jsonReplace','Require wrap!');
		switch(opt.mode){
			case 've':		opt.selector='[evar]';opt.sel='evar';break;
		}
		...
		var _render=opt.render||(opt.mode=='input'?function(jo,f,v){jo.vals(v)}:function(jo,f,v){jo.html(v)});
		thos.jwrap.find(opt.selector).each(function(){
			var jo=$(this),f=jo.attr(opt.sel);
	...
	tableReplacer:function(table,thos){
		...
		if(!thos.is__init__re){
			thos.opt=ox({progress:true,templater:true,tpl_adv:false,isloader:false,ispaging:false,append:false},thos.opt);
			thos.jwrap=thos.jwrap||$jo(opt.jwrap||opt.wrap);
		...
		if(row>0){
			...
			if(thos.jpaging && thos.vars){
				thos.jpaging.show();
				ui.paging.parser(thos.vars,thos.jpaging,function(page){thos.emit('page',page)},{style:opt.paging_style});
	...
	tips:function(jo,status,message){jo.html('<p class="itip '+status+'"><em></em><span>'+message+'</span></p>')},

	// load
	loader:function(jo){jo.html(this.loaderString())},
	loaderString:function(jo){
		var re=jo?jo.html():'';
		if(!re) re='<p class="load iloader"><em></em></p>';
		return re
	},
	loadString:function(){return '<span class="load iloading"></span>'},
	closeString:function(tag){tag=tag||'a';return '<'+tag+' class="close iclose" href="#close"></'+tag+'>'},

	loaderFiller:function(jwrap,opt){
		opt=ox({init:true,progress:true},opt);
	...
	blankString:function(opt){
		var opt_def={empty:'true',status:'info',type:'inline'};
		if(opt.isBasic){
			//data-empty="暂无记录" data-empty-type="inline" data-empty-status="info"
			var jo=opt,attr;
			opt={empty:jo.attrd('empty')};
...
vBasic.fn.extend({
	ibox:function(opt){
		if(iss(opt)){
			switch(opt){
				case 'submit':		ui.box.submit(ui.ebox);break;
				case 'close':		ui.box.close(ui.ebox);break;
			}
			return
		}
...
ui.box=function(jo,opt){return $(jo).ibox(opt)};
extendo(ui.box,{
	creater:function(jo,opt){
		if(isun(opt)){opt=jo;jo=null}
		var thos=vdcs.opter(opt);
		thos.jsrc=$jo(jo);
		thos.opt=ox({effect:'box',cover:true,autor:true},thos.jsrc?thos.jsrc.data():null,thos.opt);
		thos.loader=function(){ui.box.loader(this);this.emit('loader',this.jbox);return this};
		thos.parser=function(){ui.box.parser(this);this.emit('parser',this.jbox);return this};
		thos.run=function(){
			if(this.is_run)return this;this.is_run=true;
			this.loader();
			this.parser();
	...
	parser:function(thos){
		var opt=thos.opt,jbox=thos.jbox;

		thos.on('bar',function(real){this.jbar.sh(real)});
		thos.on('title',function(tit){if(tit) this.jbar.find('t').html(tit)});
		thos.on('btn',function(real){this.jbtn.sh(real)});
		thos.on('btn.align',function(align){if(align) this.jbtn.css('text-align',align)});

		thos.on('contenter',function(html){
			this.jcont.html(html);
			this.jbox.trigger('resizer');
			this.emit('contented');this.emit('loaded');
		});
		...
		thos.on('content',function(content){
			var htmla=(content||'').split('<!--script-->'),html=htmla[0],script=htmla[1];
		...
		thos.on('close',function(){return this.emit('closer')});
		thos.close=function(){this.emit('closer')};

		thos.jbox.on('click','.submit, a[href="#submit"], [el="submit"]',function(){thos.emit('submiter')});
		thos.jbox.on('click','.closer, a[href="#colse"], [el="close"]',function(){thos.emit('closer')});
	...
	submit:function(ebox){ebox.emit('submiter')},
	close:function(ebox){ebox.emit('closer')}
});
...
ui.pages={
	...
	exClick:function(jbtn,opt){
		if(isun(opt)){opt=jbtn;jbtn=null}
	...
	bind_include:function(serveVar,fn){
		if(iso(serveVar)) serveVar=ox({x:'e'},serveVar);
		var url=vdcs.serveURL(serveVar);
		$ajax(url,function(html){fn(html)})
	},
	...
	actionParams:function(jo,op_){
		var opt=ox({ele_item:'tr',index:'id'},op_);
		var jrow=jo.parents(opt.ele_item),id=jrow.attrd('index')||jrow.attrd(opt.index);
		var action='';url='';
		...
		return {action:action,id:id,jo:jo,jrow:jrow,url:url}
	}
};
...
ui.paging={
	def_style:'simple',
	setStyle:function(value){this.def_style=value;return this},
	parser:function(vars,jpaging,click,opt){
		opt=ox({style:this.def_style,limit:true,around:false,totals:true,jump:true},opt);
		if(opt.style=='simple' || !opt.style) opt=ox(opt,{limit:false,totals:false,jump:false});
		jpaging.htmlClear();
	...
	toString:function(opt,vars){
		var _opt={page:1,listnum:10,total:0,page_num:5,page_total:0,page_base:0,
			classname:'pagination',
			href:'javascript:;',
			limit:false,around:false,jump:false,jump_txt:'GO'};
		if(vars){
			_opt=ox(_opt,{
...
if(ui.progressi){
	NProgress.initer=function(){
		...
		NProgress._initer_=true;NProgress._doned_=true;
	...
	function f(){};ui.progressi={initer:f,istart:f,iset:f,ing:f,idone:f}
}


vBasic.fn.extend({
	taber:function(opt){return new VDCS.taber(ox({jwrap:$(this)},opt))}
});
...
VDCS.taber=function(opt){
	...
	this.initer=function(opt){
		this.opt=ox({noder:'li',selector:null,container:'body',conter:'.itab-cont',class_pop:'pop',autor:true},opt);
	...
	this.inited=function(){
		if(this.is_inited) return;this.is_inited=true;
		var that=this;
		var aselector=this.opt.selector||(this.opt.noder+' a'),n=0;
	...
	this.click=function(ja){
		...
		// key event
		var key=ja.attr('tab-key'),sn=ja.attr('tab-sn');
		var on_key=key?(key):('n'+sn);
		if(!ja.attr('tab-initer')){
			this.emit(on_key+'.init',{ja:ja,jcont:jcont,sn:sn,key:key});
			ja.attr('tab-initer','true');
		}
		this.emit(on_key,{ja:ja,jcont:jcont,sn:sn,key:key});
		return this
	};
...
var app={
	initer:function(){},
	storager:function(){this.odriver=this.odriver||vdcs.storager.driver('local');return this.odriver},
	getSession:function(key){return this.storager().get(key)},
	setSession:function(key,value){return this.storager().set(key,value)},
};
...
var ua={rc:'',id:'',names:'',sid:null,vars:{},
	location:{lng:0,lat:0,distance:'all'},
	seti:function(opt){
		extendo(this,opt);
	},
	session:function(){
	...
	loginQuick:function(opt,jo){
		opt=ox({
			...
			serveXaction:function(){return app.xlogin},
			nobtn:true,
			title:'快捷登录',
			succeed:function(){
				refresh();
