...
_proto._results = function(key,value) {
    var len = arguments.length;

    if(len === 1) {return this._taskMgr._results[key];}
    if(len === 2) {return this._taskMgr._results[key] = value;}
    return this._taskMgr._results;
};
...
_proto._run = function() {
    ...
    if(isFunction(handle)) {
        nextTick(function() {
            if(root._debug) util.logi('Start to run step: %s.', currStep.name);
