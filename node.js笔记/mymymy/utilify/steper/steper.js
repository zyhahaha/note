/*!
 * Defined by node-stepify - Stepify.js
 */
...
var Stepify = module.exports = function(result) {
    ...
    this._insetNames = ['debug', 'is', 'on', 'then', 'ok', 'succeed', 'done', 'debug', 'task', 'step', 'pend', 'error', 'timeout', 'result', 'run'];
...
Object.defineProperty(_proto, 'task', {
    ...
    value: function(taskName) {
        ...
        if(this._debug) {util.logi('Task: %s added.', taskName);}
...
_proto._errorHandle = function(err) {
    var finishHandle=this._finishHandle;
    finishHandle=this._task._taskMgr._finishHandle;
    // debugx('steper._finishHandle',this._finishHandle);
    // debugx('steper._finishHandle',this._task._taskMgr._finishHandle);
    if(finishHandle) return finishHandle({},err);
    ...
    value: function() {
        ...
        var handle = function(err) {
            if(err) {
                ...
                if(isUndefined(next)) {
                    if(this._debug) util.log_timeEnd('Tasks finished and cast');
                    ...
                    if(root._debug) util.logi('Start to run task: %s.', task._name);
                    task._run();
                }
            }
        };
        ...
        if(this._debug) util.log_time('Tasks finished and cast');
...
// VDCS.defined
Object.defineProperty(_proto, 'is', {
    writable: false,
    value: function(type) {
        var re=false;
        // debugx('type',type);
        switch(type){
            case 'succeed':
                if(this._finishHandle) re=true;
                break;
            case 'error':
                if(this._error_is) re=true;
                break;
        }
        return re
    }
...
_proto.pipEntry = function(entry,on_next,on_err) {
    entry.pipStep(this,on_next,on_err);
    return this
};
_proto.pipNext = function(next) {
    if(!next) return this;
	this
	.succeed(next)
...
Object.defineProperty(_proto, 'on', Object.getOwnPropertyDescriptor(_proto,'step'));
Object.defineProperty(_proto, 'then', Object.getOwnPropertyDescriptor(_proto,'result'));
Object.defineProperty(_proto, 'ok', Object.getOwnPropertyDescriptor(_proto,'result'));
Object.defineProperty(_proto, 'succeed', Object.getOwnPropertyDescriptor(_proto,'result'));
Object.defineProperty(_proto, 'done', Object.getOwnPropertyDescriptor(_proto,'run'));
