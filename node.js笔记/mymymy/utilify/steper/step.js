...
//     return this
// };
_proto.nexta=function(){
    var args=Array.prototype.unshift.call(arguments,null);
    return this.done.apply(args);
};
_proto.onError=_proto.error=function(err){
...
_proto.values = _proto.rets = function(key, value) {
    var len = arguments.length;

    if(len === 1) {return this._task._results(key);}
    if(len === 2) {return this._task._results(key,value);}
    return this._task._results();
};
...
_proto.go = _proto.jump = function(step) {
    ...
    if(this._debug && currStep) util.logi('The step: %s has done.', currStep.name);
...
_proto.end = function(err) {
    if(this._debug) util.logi('Task: %s has ended in the step: %s with error : %err',
        this.taskName, this.name, (err ? err.stack : null));
