...
var vBasic = Zepto = jQuery;

/* Crypt(MD5) */
(function($){
$.md5=function(s){return md5_encode(s,t)};
$.md5_hex=function(s){return hex_md5(s)};

function test_md5(){return hex_md5("abc").toLowerCase() == "900150983cd24fb0d6963f7d28e17f72"};
function md5_encode(s,t){return hex_md5(s).toLowerCase()};
...
$.query={
	...
	getString:function(){
		if(this.qstring==null){
			...
			if(this.qstring){this.qstring=this.qstring.substr(1)}
		}
		return this.qstring
	},
	getJSON:function(){
	...
	get:function(k){
		var re=this.getJSON()[k];return isun(re)?'':decodeURIComponent(re)
	},
	hash:function(v){
...
$.extend($,{
	...
	iso:function(o){return o&&o.length},
	...
	sizes:function(){return this.winSize()},
	winSize:function(){
		return {widthi:$.widthi,width:$.jwin.width(),height:$.jwin.height()}
	},
	winOffset:function(){
		return {top:$.jdoc.scrollTop(),left:$.jdoc.scrollLeft()}
	},
	testWin:function(jo){
		dbg.t('testWin');
	...
	resized:function(fn){fn($.sizes());$.resizer(fn)},
	resizer:function(fn){$.jwin.resize(function(){fn($.sizes())})},
	scrolled:function(fn){fn($.winOffset());$.scroller(fn)},
	scroller:function(fn){$.jwin.scroll(function(){fn($.winOffset())})},

	css_effect:function(real,type){
		var re='';
		switch(real){
			case 'mini':case 'popup':case 'box':
			case 'ui':		re='ui@';break;
			case 'fade':	re='fade@';break;
			case 'up':		re='fade@Up';break;
			case 'down':	re='fade@Down';break;
			case 'left':	re='fade@Left';break;
			case 'right':	re='fade@Right';break;
			default:		re=real;break;
		}
	...
	append:function(type,value,opt){
		...
		switch(type){
			...
			case 'css':			$('head').append('<style type="text/css"'+(opt.id?' id="'+opt.id+'"':'')+'>'+value+'</style>');break;
		}
		return true
	}
});
...
$.fn.extend({
	isBasic:function(){return true},
	nodeName:function(){return this[0]?this[0].nodeName.toLowerCase():''},
	htmlClear:function(){$(this).html('')},
	...
	// find
	finder:function(s){var re=this.find(s);if(re.length<1)re=null;return re},
	...
	finda:function(s,as){return this.find('a[href="'+s+'"]'+(as||''))},
	findin:function(s,as){return this.find('input[name="'+s+'"]'+(as||''))},

	// attr
	attrs:function(node){
		node=node||'data';
		var i,name,re=null,len=node.length+1,
		elem=this[0],
		attrs=elem && elem.attributes;
		if(this.length){
			re=[];
	...
	replaceClass:function(v){return this.removeClass().addClass(v)},
	...
	ontab:function(fn){$(this).on_code(9,fn)},
	onenter:function(fn){$(this).on_code(13,fn)},
	...
	ishow:function(real,fn){
		...
		if(real){
			...
			if(fn) timeoutRun(function(){fn()},1)
		}
		else{
			jo.fadeIn(500);
			if(fn) fn()
...
$.fn.extend({
	...
	formEach:function(fn){
		$(this).find($form.selector).each(function(){fn($(this))});
	},
	...
	valer:function(v){return $(this).val(v)},
	...
	disabled:function(bool){return this.form_prop('disabled',bool)},
	checked:function(bool){return this.form_prop('checked',bool)},
	selected:function(bool){return this.form_prop('selected',bool)},


	/* a[href] */
	ahref:function(){
		var jo=$(this),href=jo.attr('href'),action='';url='';
		if(href.substr(0,1)=='#') action=href.substr(1);
		else url=href;
		return {jo:jo,action:action,url:url,href:href}
	},
	aclick:function(opt,et,fn){
		if(isf(opt)){fn=opt;opt={}}
		else if(isf(et)){fn=et;et=null}
		opt=ox({selector:'a[href^="#"]'},opt);
		et=et||newEvent();
		et.jwrap=et.jwrap||$(this);
		et.jwrap.on('click',opt.selector,function(){
			var ahref=$(this).ahref(),act=ahref.action,ps={action:act,jwrap:et.jwrap,opt:opt};
			if(fn){
				fn(act,ahref.jo,ps);
				return false
			}
	...
	iclick:function(opt,et,fn){return $(this).aclick(opt,et,fn)},



	// smart
	htmlSmart:function(opt){return this.ihtml(opt)},
	itext:function(opt){return this.ihtml(opt)},
	ihtml:function(opt){
		if(!iso(opt)) opt={value:opt};
		var jo=$(this),jsub=jo.find(opt.esub||'span');
	...
	btnSmart:function(opt){
		if(!iso(opt)) opt={set:opt};
		opt=$.extend({set:'ing',value_px:'data-value',class_px:'data-class',data_orig:'orig'},opt);
		if(!opt.value_set) opt.value_set=opt.set;
		var jo=$(this),_set='',_value='',_class='',attr_orig=opt.value_px+'--'+opt.data_orig,attr_set=opt.value_px+'--set';
		if(opt.set=='reset'){
		...
		if(_value) jo.ihtml({esub:opt.esub,value:_value});
		return this
	},
	...
	placeto:function(opt){
		return this.each(function(){
			var jo=$(this);
			opt=$.extend({holder:true,offset:0,speed:800},jo.data(),opt);
			if(opt.holder) opt.offset-=$.holder_height('headbar');
			$('body').animate({scrollTop:jo.offset().top+opt.offset},opt.speed);
		});
	},
	/* float */
	floato:function(opt){
		opt=$.extend({offset:0},jo.data(),opt);
		var jo=$(this);
		var _resize=function(sizes){
			sizes=sizes||$.sizes();
			var css=ox({position:'fixed'},opt);	//,zIndex:1000
			if(typeof(opt.lefto)!=='undefined') css=ox(css,{left:((sizes.width-$.jconf.width())/2-opt.lefto-jo.width())});
			if(typeof(opt.righto)!=='undefined') css=ox(css,{right:((sizes.width-$.jconf.width())/2-opt.righto-jo.width())});
			jo.css(css).show()
		};
		jo.on('resizer',function(){_resize()});
		$.resized(_resize);
	...
	floatBox:function(opt){
		var jo=$(this);
		opt=ox({status:'',message:'hello',cover:true,top_hold:true,top_offset:100,top:0,zindex:1001},jo.data(),opt);
		...
		var _resize=function(e,sizes){
			...
			jo.css({top:_top,left:(sizes.width-jo.width())/2})
		};
		jo.on('resizer',_resize);$.resized(_resize);
		$.jfloatbox=jo;
		return jo
	},
	floatac:function(opt){
		var jo=$(this);
		opt=ox({ani:true,reset:false,offset:0,offsetX:0,offsetY:0},jo.data(),opt);
		jo.css({position:'fixed'});
		if(opt.ani) jo.addClass('anim');
		if(opt.reset) jo.css({margin:0});
		jo.on('abs_center',function(e,sizes){
			var $this=$(this);
			var _left=(sizes.width-$this.outerWidth())/2,_top=(sizes.height-$this.outerHeight())/2;
		...
		jo.show();
		$.resized(function(sizes){jo.trigger('abs_center',[sizes])});
	},


	/* linkpop */
	linkpop:function(opt){
		opt=ox({url:null,selector:'li a',father:'li',classname:'pop',attr:'href'},opt);
		if(!opt.url) opt.url=vdcs.url('path');
		var len=0,jo=null;
		$(this).find(opt.selector).each(function(){
			var ja=$(this),_url=ja.attr(opt.attr);
...
$.extend($,{
	...
	holder_height:function(t){			// headbar_holder_height
		...
		var jo=$('#'+t+'_holder');this.holder_h[t]=0;
